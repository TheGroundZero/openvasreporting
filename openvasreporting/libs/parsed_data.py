...
class ParseVulnerability:
    ...
    def __init__(self, vuln, min_level: str):
        ...
        if dolog:
            logging.debug("--------------------------------------------------------------------------------")
        if dolog:
            logging.debug("- {}".format(self.vuln_name))  # DEBUG
        if dolog:
            logging.debug("--------------------------------------------------------------------------------")
        ...
        if dolog:
            logging.debug("* vuln_id:\t{}".format(self.vuln_id))  # DEBUG
        ...
        if dolog:
            logging.debug("* vuln_cvss:\t{}".format(self.vuln_cvss))  # DEBUG
        ...
        for level in Config.levels().values():
            if self.vuln_cvss >= Config.thresholds()[level]:
                ...
                if dolog:
                    logging.debug("* vuln_level:\t{}".format(self.vuln_level))  # DEBUG
                break
        ...
        if dolog:
            logging.debug("* vuln_tags:\t{}".format(self.vuln_tags))  # DEBUG
        ...
        if dolog:
            logging.debug("* vuln_threat:\t{}".format(self.vuln_threat))  # DEBUG
        ...
        if dolog:
            logging.debug("* vuln_family:\t{}".format(self.vuln_family))  # DEBUG
        ...
        if dolog:
            logging.debug("* vuln_cves:\t{}".format(self.vuln_cves))  # DEBUG
        if dolog:
            logging.debug("* vuln_references:\t{}".format(self.vuln_references))
        # --------------------
        #
        # VULN_REFERENCES
        ...
        if dolog:
            logging.debug("* vuln_result:\t{}".format(self.vuln_result))  # DEBUG
    ...
    def check_and_parse_result(cls, vuln, config: Config):
        ...
        # is hostname included and/or excluded?
        if config.hosts_excluded is not None or config.hosts_included is not None:
            host_name = vuln.find("./host/hostname").text

            if host_name is None or host_name == "":
                host_name = "N/A"

            if config.hosts_excluded is not None and host_name in config.hosts_excluded:
                return None

            if config.hosts_included is not None and host_name not in config.hosts_included:
                return None
...
class Host(object):
    ...
    def __init__(self, ip, host_name=""):
        ...
        self.num_vulns = 0
        self.nv = {"critical": 0, "high": 0, "medium": 0, "low": 0, "none": 0}
        self.sum_cvss = 0
        self.higher_cvss = 0
        self.vuln_list = []
    ...
    def __eq__(self, other):
        return isinstance(other, Host) and other.ip == self.ip and other.host_name == self.host_name
...
class Vulnerability(object):
    ...
    def __eq__(self, other):
        ...
        if (
            other.vuln_id != self.vuln_id
            or other.name != self.name
            or other.cves != self.cves
            or other.cvss != self.cvss
            or other.level != self.level
            or other.description != self.description
            or other.detect != self.detect
            or other.insight != self.insight
            or other.impact != self.impact
            or other.affected != self.affected
            or other.solution != self.solution
            or other.solution_type != self.solution_type
            or other.references != self.references
            or other.threat != self.threat
            or other.family != self.family
            or other.result != self.result
        ):
            return False
        ...
        for key in self:
            temp_list.append(
                (
                    ...
                    self[key].nv["medium"],
                    self[key].nv["high"],
                    self[key].nv["critical"],
                    self[key].higher_cvss,
                    key,
                )
            )
        s = [v[5] for v in sorted(temp_list, key=lambda x: (x[4], x[3], x[2], x[1], x[0]), reverse=True)]
        return s
