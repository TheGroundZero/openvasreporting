...
class Config(object):
    def __init__(
        self,
        input_files,
        output_file="openvas_report",
        min_level="none",
        format="xlsx",
        report_type="host",
        template=None,
        networks_included=None,
        networks_excluded=None,
        hosts_included=None,
        hosts_excluded=None,
        regex_included=None,
        regex_excluded=None,
        ...
        :param hosts_excluded: path to file with a list of excluded host names
        :type hosts_excluded: str

        :param hosts_included: path to file with a list of included host names
        :type hosts_included: str
        ...
        if min_level.lower() in Config.levels().keys():
            ...
            raise ValueError(
                "Invalid value for level parameter, \
        ...
        self.output_file = "{}.{}".format(output_file, format) if output_file.split(".")[-1] != format else output_file
        ...
        if report_type == "v" or report_type == "vulnerability":
            ...
            raise ValueError(
                "Expected host or vulnerability for report-type parameter but got '{}' instead.".format(report_type)
            )

        if networks_excluded is not None:
        ...
        if networks_included is not None:
        ...
        if hosts_excluded is not None:
            with open(hosts_excluded) as f:
                lines = f.read().splitlines()
            self.hosts_excluded = lines
        else:
            self.hosts_excluded = None

        if hosts_included is not None:
            with open(hosts_included) as f:
                lines = f.read().splitlines()
            self.hosts_included = lines
        else:
            self.hosts_included = None

        if regex_excluded is not None:
    ...
    @staticmethod
    def levels():
        return {"c": "critical", "h": "high", "m": "medium", "l": "low", "n": "none"}

    @staticmethod
    def thresholds():
        return {"critical": 9.0, "high": 7.0, "medium": 4.0, "low": 0.1, "none": 0.0}
...
class Config_YAML(Config):
    def __init__(self, input_files, config_file, output_file="openvas_report"):
        ...
        self.output_file = (
            "{}.{}".format(output_file, self.format) if output_file.split(".")[-1] != self.format else output_file
        )
